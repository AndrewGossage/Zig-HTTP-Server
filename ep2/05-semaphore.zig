...
fn listen(io: std.Io, gpa: std.mem.Allocator) !void {
    var sem: std.Io.Semaphore = .{ .permits = 100 };
    ...
    while (true) {
        ...
        try sem.wait(io);
        try group.concurrent(io, handle_connection, .{ io, &sem, gpa, stream });
    }
}

fn handle_connection(io: std.Io, sem: *std.Io.Semaphore, gpa: std.mem.Allocator, stream: std.Io.net.Stream) !void {
